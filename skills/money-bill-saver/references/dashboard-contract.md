...
For a status-check action, prefer a known official account or billing entry in `action.url`. A general merchant account homepage may be labeled “Possible account entry” when the precise setting is not established. If no suitable URL is known, keep the navigation instruction in `action.summary` or `steps` and leave the URL absent. A missing URL is a visible report gap, not a reason to search every merchant again or fail the report. Verify a route before using it to submit a financial request or alter an account.
...
Functional overlap means a possible shared job or workflow, not identical products or proven duplicate subscriptions. A plausible shared purpose is enough for a tentative lead; identify the peer and say which one currently looks preferable to keep, or state the smallest use/feature check that would choose between them. Label that preference provisional. Verify actual use, required features, prices and exit dependencies before recommending cancellation or estimating future savings. `usage_unverified` can flag a paid service with no established recent use, or a long gap in the available reviewed records. State the last supported event and reviewed coverage rather than claiming the whole mailbox was silent or that charges continued throughout the gap. Product-update email is neither account activity nor proof of non-use. `trial_conversion` flags an announced conversion whose paid outcome needs checking; it does not imply that a charge occurred.
