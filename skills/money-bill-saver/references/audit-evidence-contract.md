...
Independent queries may run concurrently. Preserve each query's page-token chain and raw results separately, then deduplicate returned messages by mailbox and message ID before source triage; concurrent execution does not relax the completion checks. The current manifest represents service-scoped searches; retain account-wide candidate searches and their private metadata index as separate discovery records, then associate merchant searches and sources with real report services or cases. The manifest gate does not check those account-wide records: an unfinished candidate page chain or plausible untriaged sender group requires an explicitly provisional overall mailbox finding even if the service-scoped gate passes. Do not fabricate `service_ids` for an empty inventory.
