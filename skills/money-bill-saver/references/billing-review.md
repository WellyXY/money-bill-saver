...
Prefer the user's selected mailbox invoices when an authorized mailbox read tool is available. Inspect the tool's actual search, pagination, message and attachment capabilities. Search the authorized folders and dates using billing and lifecycle concepts in the source mailbox's languages. Start with every user-named or already evidenced merchant, then use high-signal purchase, invoice/receipt, welcome, trial and renewal queries to find others. These discovery queries are not proof of exhaustive mailbox coverage. Avoid account-wide free-text queries built from generic words such as `plan`, `payment`, `charge` or `credit` unless a specific coverage gap justifies them; narrow such terms by subject, sender or merchant. Include related billing threads, complete the chosen query pages, and report truncation or inaccessible attachments. English report defaults do not restrict the languages of source evidence or searches.

For a broad mailbox review, the coordinating agent fixes the authorized account, folders and date window before searching. Prefer a host search that returns ID, sender, subject, date, snippet and attachment flag (`gmail_search_emails` when available) over an ID-only search followed by bulk full-message reads. Save each raw page on first retrieval in a private task-owned directory (`0700` directory, `0600` files), with exact query, request page token, result and next-page token; label the mailbox when more than one account is in scope. Follow tokens in order within each query while running independent short query chains concurrently. Never repeat a completed search just to save its results. Build one [index_mail_candidates.py](../scripts/index_mail_candidates.py) index per mailbox, deduplicating message IDs within it, then group saved metadata by sender and subject pattern. Read its bounded group summary first, then selected IDs and details from the private full index; if any count in `omitted_groups` exceeds zero, inspect that tail locally before claiming discovery complete. Do not paste thousands of message rows into model context. Missing metadata or an incomplete page chain stays unresolved; a snippet cannot establish an invoice date, successful payment, current plan or absence.

Triage sender/merchant groups first. Mark clearly unrelated metadata with a specific reason, and fetch full messages and relevant attachments only for financially plausible or ambiguous billing and lifecycle candidates. Keep account-wide candidate queries and the group ledger as discovery working records: the current evidence manifest requires real `service_ids` for every search and returned source. If a candidate query has an unfinished or unverifiable page chain, report that scope as provisional. Every user-named or plausible merchant group must become a report service/case or an explicit unresolved/excluded group before rendering; never silently drop a group. If several merchants need substantial investigation, assign different groups to up to three workers using the same authorized host mail tool. Each worker completes broad brand/seller/sender searches without billing-keyword restrictions, checks payment and top-up history plus later state changes, saves reviewed sources and returns structured facts and gaps. For a shared payment processor or app store, split results by actual seller/account rather than treating all processor mail as one service-scoped search. The coordinator merges duplicate sources, reconciles cross-merchant payment channels and writes the final `audit-evidence.json`. Do not invent service IDs or treat the manifest check as proof that account-wide discovery was complete; unresolved plausible groups make the overall mailbox finding provisional.
...
2. Follow every available result page. Read the relevant full messages and identified invoice/receipt attachments, preserving message and document dates separately. Keep account-wide candidate queries, scope and page completion in discovery working records. Record service-scoped merchant queries, scope, page completion and unread/truncated items in `audit-evidence.json` using [audit-evidence-contract.md](audit-evidence-contract.md). A file-only review records the complete supplied file scope instead of claiming a mailbox search.
...
If reporting a historical payment or top-up total, enumerate the distinct successful transactions within the declared scope; a latest receipt is not the entire history. Keep receipt payment dates separate from message arrival and invoice issue dates. For each service, fill last invoice only from a reviewed invoice's issue date; if the invoice or its attachment was not read, state that gap instead of substituting the notification date.
