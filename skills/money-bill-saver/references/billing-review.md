...
For a broad mailbox review, the coordinating agent first fixes the authorized account, folders, date window and initial query families. When the host supports subagents and enough independent work exists to outweigh their startup time, assign distinct candidate query families across the full date window to a small worker group (initially up to three workers). Give each worker a separate evidence output path. Workers use the same authorized host mail tool, save the exact query and raw result for every page, and return message IDs, page completion and retrieval gaps. Do not send credentials or full private messages between agents. Follow each query's page tokens in order; different query chains can run concurrently. Respect connector throttling; for short searches, prefer concurrent calls in the coordinating agent when available.

The coordinator merges candidates by mailbox and message ID before fetching relevant full messages. Keep account-wide candidate queries as discovery working records: the current evidence manifest requires real `service_ids` for every search and returned source. Once merchants are identified, assign different merchants' broad discovery and lifecycle searches concurrently under the same scope, and include those service-scoped searches and their results in `audit-evidence.json`. Do not invent service IDs or silently treat the manifest check as proof that the account-wide candidate pass was complete. If no report service or case is identified, disclose the candidate search and explicitly mark the overall mailbox finding provisional, even if an empty manifest passes its structural check. The coordinator owns source dispositions, the final manifest, later-event reconciliation and all status or absence conclusions. A worker's empty result or unfinished page chain does not establish that a service is absent.
