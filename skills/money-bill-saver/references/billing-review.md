...
4. Read `body.txt` and material attachment text, preserving table rows and column relationships. Inspect PDF integrity metadata first. Read original pages when text is sparse/garbled, a parser warns, signs or columns are ambiguous, amounts fail reconciliation, or material dates/status conflict. Inspect scans and relevant images, including inline invoice images, and check attached `.eml` contents. Reliable extracted text can support review without re-rendering every page; record the actual review method. For a second local extraction, run `extract_pdf.py <PDF> --backend pdftotext --output-dir <fresh directory>` when Poppler is available. Preserve both extracts and resolve disagreements against the original; a clean second parse does not erase the first warning or prove correctness. An encrypted PDF blocks that attachment, while other documents continue; request an unlocked copy.
