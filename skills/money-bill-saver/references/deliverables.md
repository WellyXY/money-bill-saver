...
Read [dashboard-contract.md](dashboard-contract.md) for the renderer schema and validation boundary. The report page presents these three sections in order:
...
Each item opens the supporting timeline, unresolved facts, a known official action or status-check entry when available, and a copyable draft when one exists. A possible status entry can be labeled as such; if no entry is known, state where to look. Do not do a separate lookup per row just to fill the link. A service remains in the first section when it also has a refund question or other issue. Include non-subscription refund cases in the refund section only when they have a specific sourced refund basis; other non-subscription cases go to other issues. Source type alone does not decide the issue group.
